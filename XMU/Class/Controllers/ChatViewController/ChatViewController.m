...
//    [backItem release];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(messageChange:) name:@"didReceiveMessage" object:nil];
}
...
{   
    ...
    XMPPMessage *mesa = [XMPPMessage messageWithType:@"chat" to:tojid elementID:[[NSUserDefaults standardUserDefaults] stringForKey:kXMPPmyJID]];
    [mesa addChild:body];
...
- (void) messageChange:(NSNotification *)sender
{
    NSDictionary *t_dic = sender.userInfo;
    NSString *msg = [t_dic objectForKey:@"msg"];
    NSString *name = [t_dic objectForKey:@"name"];
    NSLog(@"%@",msg);
    UIView *chatView = [self bubbleView:[NSString stringWithFormat:@"%@:%@",name, msg]
								   from:NO];
    
	[self.chatArray addObject:[NSDictionary dictionaryWithObjectsAndKeys:msg, @"text", name, @"speaker", chatView, @"view", nil]];
...
-(void) autoMovekeyBoard: (float) h{
    ...
    [UIView animateWithDuration:0.3f animations:^(){
        toolbar.frame = CGRectMake(0.0f, (float)(__MainScreen_Height+20-h-108.0), 320.0f, 44.0f);
        tableView.frame = CGRectMake(0.0f, 0.0f, 320.0f,(float)(__MainScreen_Height+20-h-108.0));
    }];
}
...
//    UIGestureRecognizer *gest = [[UIGestureRecognizer alloc] initWithTarget:self action:@selector(hideKeyBoard)];
//    [_chatTableView addGestureRecognizer:gest];
    UITapGestureRecognizer* singleRecognizer;
    singleRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideKeyBoard)];
    //点击的次数
    [_chatTableView addGestureRecognizer:singleRecognizer];
...
//                UIWebView *webView = [[UIWebView alloc] initWithFrame:CGRectMake(upX, upY, KFacialSizeWidth, KFacialSizeHeight)];
